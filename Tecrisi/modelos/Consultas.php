...
Class Consultas
{
	...
	public function consultasiniestrosfechaseguradora($fecha_inicio,$fecha_fin,$idcliente)
	{
		$sql="select * from statustragicos st inner join tragico t on t.idmovint=st.idsiniestro inner join aseguradora aseg on aseg.idseguradora=t.aseguradora 
		inner join usuario u on u.idusuario=t.analista 
		
		WHERE u.cargo='analistas' AND DATE(st.fechacambio)>='$fecha_inicio' AND DATE(st.fechacambio)<='$fecha_fin' AND aseg.idseguradora='$idcliente'";
		return ejecutarConsulta($sql);		
	}
